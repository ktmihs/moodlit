...
router.post('/', validate({ body: postBody }), async (req, res) => {
	...
	if (!(review as { content: string | null }).content)
		return void apiError(res, ErrorCode.MISSING_PARAM, '분석할 감상이 없습니다.');
	...
	const age = cached
		? Date.now() - new Date((cached as { cached_at: string }).cached_at).getTime()
		: Infinity;
		...
		buildRecommendations(review_id, user.id, supabase)
			.then(recommendations =>
				admin.from('recommendations').upsert(
					{ review_id, results: recommendations, cached_at: new Date().toISOString() },
	...
	try {
		const recommendations = await buildRecommendations(review_id, user.id, supabase);
